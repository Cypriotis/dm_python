...
class manageDuplicateInputs:
    def execute(t_name, col1):
        ...
        columns_to_check_duplicates = [col1]
...
class manageOutlierInputs:
    def execute(t_name, col1):
        ...
        df.loc[(df[col1] < lower_bound) | (
            df[col1] > upper_bound), col1] = None
...
class trendSpotter:
    def execute():
        ...
        # Replace 'date' with the actual column name
        ...
        seasonality.to_csv(os.path.join(
            export_dir, 'seasonality.csv'), index=True)
